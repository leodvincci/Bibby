...
public class PromptOptions {
  ...
  public List<SelectItem> authorOptions(AuthorDTO author) {
    ...
    List<AuthorDTO> authors =
        authorFacade.getAllAuthorsByName(author.firstName(), author.lastName());
  ...
  public Map<String, String> bookCaseOptions() {
    ...
    for (BookcaseDTO bookcaseDTO : bookcaseDTOs) {
      ...
      for (ShelfDTO s : shelves) {
        List<BookDTO> bookList = bookFacade.findByShelfId(s.shelfId()).stream().map(
                book -> {
                  return new BookDTO(
                          book.getBookId().getId(),
                          book.getEdition(),
                          book.getTitle().title(),
                          book.getAuthors().stream().map(a -> a.getAuthorFirstName() + " " + a.getAuthorLastName()).toList(),
                          book.getIsbn().isbn,
                          book.getGenre(),
                          book.getPublisher(),
      ...
      options.put(
          bookcaseRowFormater(bookcaseDTO, shelfBookCount), bookcaseDTO.bookcaseId().toString());
    }
    return options;
  }
