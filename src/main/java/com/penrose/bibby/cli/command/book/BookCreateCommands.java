...
public class BookCreateCommands {
  ...
  public BookCreateCommands(
      CliPromptService cliPrompt, BookFacade bookFacade, AuthorFacade authorFacade) {
  ...
  /**
   * Registers a new book entry in the library system. Depending on the provided options, this
   * method can either scan a book's ISBN for metadata retrieval or allow manual entry of book
   * details.
   *
   * @param scan if true, initiates a scanning process to retrieve book metadata; if false, allows
   *     manual entry of book details.
   * @param multi if true, enables multiple book scanning functionality (currently unused); if
   *     false, processes a single book entry.
   */
  //    @Command(command = "register"
  //            , description = """
  //                            \u001B[38;5;185mAdd books to your library by ISBN (scan/type/paste)
  // or manual entry. If an ISBN is provided, Bibby fetches metadata and creates a new book record.
  ...
  /**
   * Manually creates a new book entry by prompting the user for input. This method facilitates the
   * interactive addition of a book, including its title, authors, and ISBN, by collecting details
   * directly from the user and validating the information. The collected data is then used to
   * create a BookRequestDTO, which is passed to the book facade for further processing and storage.
   *
   * <p>The following steps are performed: 1. Prompts the user to input the book title. 2. Invokes
   * the createAuthors method to gather and process author details. 3. Prompts the user to input the
   * book's ISBN. 4. Logs the collected details (title, authors, and ISBN). 5. Constructs a
   * BookRequestDTO object encapsulating these details. 6. Uses the book facade to create a new book
   * entry in the system.
   */
