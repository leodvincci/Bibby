 ...
 * such as creation and last-update timestamps.
 *
 * <h2>Purpose</h2>
 * <ul>
 *   <li>Provide a single domain object to represent and evolve a booklist over time.</li>
 *   <li>Encapsulate rename behavior so timestamp updates happen consistently.</li>
 * </ul>
 *
 * <h2>Invariants</h2>
 * <ul>
 *   <li>{@code listId} is required and must not change after construction.</li>
 *   <li>{@code booklistName} is required; renaming to {@code null} is rejected.</li>
 *   <li>{@code createdAt} represents the creation time and should not move forward/backward once set.</li>
 *   <li>{@code updatedAt} is the last modification time and should be {@code >= createdAt}.</li>
 *   <li>{@code bookIdentifier} represents the current membership of the list; callers should treat it
 *       as a set (no duplicates) and avoid {@code null} elements.</li>
 * </ul>
 *
 * <p><strong>Note:</strong> Some invariants (e.g., non-null fields, timestamp ordering) are domain rules
 * and should be enforced by construction and mutation methods; setters that allow arbitrary values may
 * bypass these guarantees.</p>
 */
public class Booklist {
    ...
    public Booklist(BooklistId listId, BooklistName booklistName, Set<BookIdentifier> bookIdentifier) {
