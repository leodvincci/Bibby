...
public class AuthorRepositoryImpl implements AuthorRepository {
   ...
   * A Book may have zero, one, or multiple Authors. This method resolves that relationship by
   * querying the underlying persistence store for all Author records linked to the specified Book
   * ID.
   *
   * <p><strong>Contract:</strong>
   *
   * <ul>
   *   <li>Returns an immutable empty Set if the Book has no associated Authors.
   *   <li>Never returns {@code null}.
   *   <li>Always returns domain-level {@code Author} objects, not entities.
   *   <li>If the Book ID does not exist, the result is also an empty Set.
   * </ul>
   *
   * @param id the unique identifier of the Book whose Authors should be retrieved; must not be
   *     {@code null}.
   * @return a Set of domain {@code Author} objects associated with the Book, or an empty Set if
   *     none exist
   */
  @Override
  ...
  public Optional<Author> findAuthorById(Long id) {
    ...
    return Optional.of(
        AuthorMapper.toDomain(id, authorEntity.getFirstName(), authorEntity.getLastName()));
  }

  /**
   * Creates and persists a new Author in the system.
   *
   * <p>This operation constructs a new Author from the provided first and last name, saves it to
   * the underlying persistence store, and returns the newly-created domain Author.
   *
  ...
  public Optional<Author> createAuthor(String authorFirstName, String authorLastName) {
    ...
    Optional<AuthorEntity> newAuthorEntity =
        authorJpaRepository.getByFirstNameAndLastName(authorFirstName, authorLastName);
    return Optional.of(
        AuthorMapper.toDomain(
            newAuthorEntity.get().getAuthorId(),
            newAuthorEntity.get().getFirstName(),
            newAuthorEntity.get().getLastName()));
  }
  ...
  public Optional<Author> getByFirstNameAndLastName(String firstName, String lastName) {
    AuthorEntity authorEntity =
        authorJpaRepository.getByFirstNameAndLastName(firstName, lastName).orElse(null);
    if (authorEntity != null) {
      return Optional.of(
          AuthorMapper.toDomain(
              authorEntity.getAuthorId(), authorEntity.getFirstName(), authorEntity.getLastName()));
    }
  ...
  public List<AuthorDTO> findAllByFirstNameLastName(String firstName, String lastName) {
    List<AuthorEntity> authorEntities =
        authorJpaRepository.getAllByFirstNameAndLastName(firstName, lastName);
    return authorEntities.stream()
        .map(
            entity ->
                AuthorMapper.toDTO(
                    AuthorMapper.toDomain(
                        entity.getAuthorId(), entity.getFirstName(), entity.getLastName())))
        .toList();
  }
